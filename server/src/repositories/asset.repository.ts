...
export class AssetRepository implements IAssetRepository {
  ...
  async upsertJobStatus(...jobStatus: (Partial<AssetJobStatusEntity> & { assetId: string })[]): Promise<void> {
    ...
    await this.db
      ...
      .onConflict((oc) =>
        oc.columns(['assetId']).doUpdateSet(() => {
          delete (jobStatus[0] as any)['assetId'];
  ...
  async getWithout(pagination: PaginationOptions, property: WithoutProperty): Paginated<AssetEntity> {
    const items = await this.db
      ...
      .$if(property === WithoutProperty.SMART_SEARCH, (qb) =>
        qb.where((eb) =>
          eb.not((eb) => eb.exists(eb.selectFrom('smart_search').where('assetId', '=', eb.ref('assets.id')))),
        ),
      )
      ...
      .with('assets', (qb) =>
        qb
          ...
          .$if(!!options.personId, (qb) =>
            qb.innerJoin(
              () => sql.table('has_people').as('has_people'),
  ...
  async getTimeBucket(timeBucket: string, options: TimeBucketOptions): Promise<AssetEntity[]> {
    ...
    return withExif(query)
      ...
      .$if(!!options.userIds, (qb) => qb.where('ownerId', '=', anyUuid(options.userIds!)))
      .$if(options.isArchived !== undefined, (qb) => qb.where('isArchived', '=', options.isArchived!))
      .$if(options.isFavorite !== undefined, (qb) => qb.where('isFavorite', '=', options.isFavorite!))
      .$if(!!options.withStacked, (qb) => withStack(qb, { assets: true })) // TODO: optimize this; it's a huge performance hit
      .$if(!!options.assetType, (qb) => qb.where('type', '=', options.assetType!))
      .$if(options.isDuplicate !== undefined, (qb) =>
        qb.where('duplicateId', options.isDuplicate ? 'is not' : 'is', null),
      )
      .where('deletedAt', options.isTrashed ? 'is not' : 'is', null)
      .where('isVisible', '=', true)
      .where(
